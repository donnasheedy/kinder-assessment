...
/**
 * Subtraction - Subtraction assessment with parts
 * type="single": Part 1 (0-5), Part 2 (0-9)
 * type="double": Part 1 (10-20 - 1-digit), Part 2 (10-90 - 1-digit)
 */
export default function Subtraction({ type = 'single' }) {
    ...
    const TYPE_SETTINGS = {
        single: {
            ...
            parts: [
                { num: 1, label: 'Part 1: 0-5', desc: 'Subtract numbers 0-5', max: 5 },
                { num: 2, label: 'Part 2: 0-9', desc: 'Subtract numbers 0-9', max: 9 }
            ]
        },
        double: {
    ...
    // Generate 10 random subtraction problems (ensuring no negative answers)
    const generateProblems = useCallback(() => {
        ...
        while (probs.length < NUM_PROBLEMS) {
            ...
            if (type === 'double') {
                ...
                // Single digit: 0 to max, ensure a >= b
                a = Math.floor(Math.random() * (currentPart.max + 1));
                b = Math.floor(Math.random() * (a + 1)); // b is always <= a
            }

            // For double digit, ensure a > b for positive result
            if (type === 'double' && b >= a) {
                continue;
            }
